...
export const appendStatusToResponse = (taggedGigsArray: Array<taggedGigs>, simplifiedGigArray: Array<SimplifiedGig>) => {
    ...
    taggedGigsArray.map((g: { gigId: string, status: string }) => {
        ...
        const index: number = simplifiedGigArray.findIndex(item => item.id === g.gigId)
