...
const signup = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const duplicateUsername = (await UserQuery(
      connection,
      duplicateUserQuery
    )) as any;
  ...
  const duplicateNameQuery =
    `SELECT EXISTS(SELECT "username" FROM users WHERE name = ` +
    connection.escape(req.body.name) +
    `) as TRUTH`;

  try {
    const duplicateName = (await UserQuery(
      connection,
      duplicateNameQuery
    )) as any;
    ...
    connection.end();
    return res.status(400).json({ message: error.message, field: "name" });
  }

  try {
    const hashedPassword = await argon2.hash(req.body.password);
    //Escape the hashed password since it can contain special characters that confuse the query
    const insertNewUserquery = `INSERT INTO users (username, password, name) VALUES (${connection.escape(
      req.body.username.toLowerCase()
    )},${connection.escape(hashedPassword)},${connection.escape(
      req.body.name
    )})`;
...
const addFriend = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const newFriendExistsQuery = `SELECT EXISTS(SELECT name FROM users WHERE name = ${connection.escape(
      friendToAdd
    )}) as TRUTH`;
    const newFriendExistsResult = await UserQuery(
      connection,
      newFriendExistsQuery
    );
    if (newFriendExistsResult[0].TRUTH === 0)
      throw new Error("User named " + friendToAdd + " does not exist.");

    const userIsFriendToAddQuery = `SELECT id FROM users WHERE name = ${connection.escape(
      friendToAdd
    )}`;
    const userIsFriendToAddResult = await UserQuery(
      connection,
      userIsFriendToAddQuery
    );
    if (userIsFriendToAddResult[0].id === friendRequester)
      throw new Error("You cannot add yourself as your friend.");

    const duplicateFriendQuery =
      `SELECT EXISTS(SELECT id FROM friends WHERE friendName = ` +
      connection.escape(friendToAdd) +
      ` AND userId = ` +
      connection.escape(friendRequester) +
      `) as TRUTH`;
    const duplicateFriendResult = (await UserQuery(
      connection,
      duplicateFriendQuery
    )) as any;
    if (duplicateFriendResult[0].TRUTH === 1)
      throw new Error(
        "User named " + friendToAdd + " is already on your friends list."
      );
    ...
    return res.status(201).json("New friend added succesfully");
  } catch (error: any) {
    return res
      .status(400)
